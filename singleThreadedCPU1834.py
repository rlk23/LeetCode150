...
class Solution:
    def getOrder(self, tasks: List[List[int]]) -> List[int]:
        # Add index to each task for tie-breaking
        ...
        current_time = 0
        task_index = 0  # Index to track the sorted tasks
        
        while len(result) < len(tasks):
            # Add all tasks that are available at current_time to the heap
            while task_index < len(tasks) and indexed_tasks[task_index][0] <= current_time:
                enqueue, process, idx = indexed_tasks[task_index]
                heappush(min_heap, (process, idx))  # Push by processing time, then index
                task_index += 1
            
            # If heap is empty, jump to the next task's enqueue time
            if not min_heap:
                current_time = indexed_tasks[task_index][0]
                continue
            
            # Process the next task in the heap
            process_time, idx = heappop(min_heap)
            result.append(idx)
            current_time += process_time  # Update time after processing the task
